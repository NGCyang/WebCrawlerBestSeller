...
@app.route('/bestseller/category/<category_id>', methods=['GET'])
def get_category_bestseller(category_id = None):
    cursor = collection.find({ "categorys" : category_id }).sort("storage",ASCENDING).limit(20)
    return json_util.dumps(cursor)
